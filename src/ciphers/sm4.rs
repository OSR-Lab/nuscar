...
use numpy::{PyArray2, PyArray3, PyReadonlyArray1, PyReadonlyArray2, ToPyArray, PyArrayMethods};
use pyo3::prelude::*;
...
fn sm4_t(a: u32) -> u32 {
    ...
    SBOX_T0[buf[0] as usize]
        ^ SBOX_T1[buf[1] as usize]
        ^ SBOX_T2[buf[2] as usize]
        ^ SBOX_T3[buf[3] as usize]
}
...
#[pyfunction]
pub fn sm4_attack_first_sbox_value_with_guess_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    guesses: Vec<u8>,
    pos: Vec<usize>, // key byte position
) -> Bound<'py, PyArray3<u8>> {
    let plain_r = plaintext.as_array();
    let (rows, _cols) = plain_r.dim();

    let mut res: Array3<u8> = Array::zeros((rows, guesses.len(), pos.len()));
    p.on_worker(_py, || {
        Zip::indexed(&mut res).par_for_each(|(i, j, k), z| {
            let x = plain_r[[i, pos[k] + 4]]
                ^ plain_r[[i, pos[k] + 8]]
                ^ plain_r[[i, pos[k] + 12]]
                ^ guesses[j];
            *z = SBOX[x as usize];
...
#[pyfunction]
pub fn sm4_attack_first_sbox_hw_with_guess_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    guesses: Vec<u8>,
...
#[pyfunction]
pub fn sm4_attack_first_sbox_bit_with_guess_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    guesses: Vec<u8>,
...
#[pyfunction]
pub fn sm4_attack_sbox_value_with_guess_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    rk: PyReadonlyArray2<u8>,        // round key
    guesses: Vec<u8>,
    pos: Vec<usize>, // key byte position
) -> Bound<'py, PyArray3<u8>> {
    let mut plain_r = plaintext.to_owned_array();
    let (rows, _cols) = plain_r.dim();
    let mut res: Array3<u8> = Array::zeros((rows, guesses.len(), pos.len()));
    let rk_r = rk.as_array();
    let round = rk_r.dim().0;
    p.on_worker(_py, || {
        plain_r
            ...
            .for_each(|mut plain| {
                for i in 0..round {
                    let p = plain.as_slice_mut().unwrap();

                    let rk_row = rk_r.row(i);
                    let rk_slice = rk_row.as_slice().unwrap();
                    let x0 = BigEndian::read_u32(&p[0..4]);
                    let x1 = BigEndian::read_u32(&p[4..8]);
                    let x2 = BigEndian::read_u32(&p[8..12]);
                    let x3 = BigEndian::read_u32(&p[12..16]);
                    let k = BigEndian::read_u32(rk_slice);
                    let x4 = sm4_t(x1 ^ x2 ^ x3 ^ k) ^ x0;
                    p[0..4].copy_from_slice(x1.to_be_bytes().as_slice());
                    p[4..8].copy_from_slice(x2.to_be_bytes().as_slice());
                    p[8..12].copy_from_slice(x3.to_be_bytes().as_slice());
                    p[12..16].copy_from_slice(x4.to_be_bytes().as_slice());
                }
            });
        Zip::indexed(&mut res).par_for_each(|(i, j, k), z| {
            let x = plain_r[[i, pos[k] + 4]]
...
#[pyfunction]
pub fn sm4_attack_sbox_hw_with_guess_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    rk: PyReadonlyArray2<u8>,        // round key
...
#[pyfunction]
pub fn sm4_attack_sbox_bit_with_guess_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    rk: PyReadonlyArray2<u8>,        // round key
...
#[pyfunction]
pub fn sm4_encrypt_fix_key_step_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    ...
    let mut round = at_round;
    if step == Steps::Rout || step == Steps::FinalX {
        round += 1;
    }
    p.on_worker(_py, || {
        (plain_r.axis_iter_mut(Axis(0)), res.axis_iter_mut(Axis(0)))
            .into_par_iter()
            .for_each(|(mut plain, mut r)| {
                for i in 0..round {
                    let p = plain.as_slice_mut().unwrap();
                    let x0 = BigEndian::read_u32(&p[0..4]);
                    let x1 = BigEndian::read_u32(&p[4..8]);
                    let x2 = BigEndian::read_u32(&p[8..12]);
                    let x3 = BigEndian::read_u32(&p[12..16]);
                    let x4 = sm4_t(x1 ^ x2 ^ x3 ^ sm4.rk[i]) ^ x0;
                    p[0..4].copy_from_slice(x1.to_be_bytes().as_slice());
                    p[4..8].copy_from_slice(x2.to_be_bytes().as_slice());
                    p[8..12].copy_from_slice(x3.to_be_bytes().as_slice());
                    p[12..16].copy_from_slice(x4.to_be_bytes().as_slice());
                ...
                let x3 = BigEndian::read_u32(&p[12..16]);
                let x4: u32;
                let slicer = r.as_slice_mut().unwrap();
                if step != Steps::Rout && step != Steps::FinalX {
                    let k = sm4.rk[round];
                    match step {
                        Steps::AK => {
                        ...
                        Steps::XorX0 => {
                            x4 = sm4_t(x1 ^ x2 ^ x3 ^ k) ^ x0;
                        }
                        _ => {
                            panic!("step error.");
                        }
                    }
                    slicer.copy_from_slice(x4.to_be_bytes().as_slice());
                }
                if step == Steps::FinalX {
                    if at_round != 31 {
                        panic!("FinalX only at last round.");
                    }
                    p[0..4].copy_from_slice(x3.to_be_bytes().as_slice());
                    p[4..8].copy_from_slice(x2.to_be_bytes().as_slice());
                    p[8..12].copy_from_slice(x1.to_be_bytes().as_slice());
                    p[12..16].copy_from_slice(x0.to_be_bytes().as_slice());
                }
            });
    });
    if step != Steps::Rout && step != Steps::FinalX {
        ...
        plain_r.to_pyarray_bound(_py)
    }
}

#[pyfunction]
pub fn sm4_decrypt_fix_key_step_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    ciphertext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    key: PyReadonlyArray1<u8>,
    at_round: usize,
    at_step: u8,
) -> Bound<'py, PyArray2<u8>> {
    let mut cipher_r = ciphertext.to_owned_array();
    let key = key.as_slice().unwrap();
    let sm4 = Sm4::new(key);
    let step: Steps = Steps::from(at_step);
    let (rows, _cols) = cipher_r.dim();
    let mut res: Array2<u8> = Array::zeros((rows, 4));
    let mut round = at_round;
    if step == Steps::Rout || step == Steps::FinalX {
        round += 1;
    ...
    p.on_worker(_py, || {
        (cipher_r.axis_iter_mut(Axis(0)), res.axis_iter_mut(Axis(0)))
            .into_par_iter()
            .for_each(|(mut cipher, mut r)| {
                for i in 0..round {
                    let p = cipher.as_slice_mut().unwrap();
                    let x0 = BigEndian::read_u32(&p[0..4]);
                    let x1 = BigEndian::read_u32(&p[4..8]);
                    let x2 = BigEndian::read_u32(&p[8..12]);
                    let x3 = BigEndian::read_u32(&p[12..16]);
                    let x4 = sm4_t(x1 ^ x2 ^ x3 ^ sm4.rk[31 - i]) ^ x0;
                    p[0..4].copy_from_slice(x1.to_be_bytes().as_slice());
                    p[4..8].copy_from_slice(x2.to_be_bytes().as_slice());
                    p[8..12].copy_from_slice(x3.to_be_bytes().as_slice());
                    p[12..16].copy_from_slice(x4.to_be_bytes().as_slice());
    ...
    if step != Steps::Rout && step != Steps::FinalX {
        ...
        cipher_r.to_pyarray_bound(_py)
    }
}


#[pyfunction]
pub fn sm4_encrypt_step_r<'py>(
    _py: Python<'py>,
    p: &ThreadPool,
    plaintext: PyReadonlyArray2<u8>, // intermediate data accumulator with shape (batchsize, nb_data)
    ...
    p.on_worker(_py, || {
        (
            plain_r.axis_iter_mut(Axis(0)),
            res.axis_iter_mut(Axis(0)),
            key_r.axis_iter(Axis(0)),
        )
            .into_par_iter()
            .for_each(|(mut plain, mut r, k)| {
                let sm4 = Sm4::new(k.as_slice().unwrap());
                for i in 0..round {
                    let p = plain.as_slice_mut().unwrap();
                    let x0 = BigEndian::read_u32(&p[0..4]);
                    let x1 = BigEndian::read_u32(&p[4..8]);
...
pub fn sm4_decrypt_step_r<'py>(
    ...
    p.on_worker(_py, || {
        (
            cipher_r.axis_iter_mut(Axis(0)),
            res.axis_iter_mut(Axis(0)),
            key_r.axis_iter(Axis(0)),
        )
            .into_par_iter()
            .for_each(|(mut cipher, mut r, k)| {
                let sm4 = Sm4::new(k.as_slice().unwrap());
                for i in 0..round {
                    let p = cipher.as_slice_mut().unwrap();
                    let x0 = BigEndian::read_u32(&p[0..4]);
...
pub fn sm4_opl_r<'py>(
    ...
    let mut input_r = input.to_owned_array();
    p.on_worker(_py, || {
        (input_r.axis_iter_mut(Axis(0)))
            .into_par_iter()
            .for_each(|mut d| {
                let p = d.as_slice_mut().unwrap();
                let x = BigEndian::read_u32(&p[0..4]);
                let y = el(x);
                p.copy_from_slice(y.to_be_bytes().as_slice());
            });
    });
    input_r.to_pyarray_bound(_py)
}
